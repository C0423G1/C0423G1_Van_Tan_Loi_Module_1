...
function LoginForm({toggleForm}) {
    ...
    const faceIoScriptLoaded = () => {
        ...
        if (faceIO && !faceioInstance) {
            faceioInstance = new faceIO('fioa1a1a')
        }
    }
    ...
    const faceRegistration = async () => {
        try {
            const userInfo = await faceioInstance.enroll({
                locale: "auto",
                payload: {
                    email: "hongnguyen.1996@gmail.com",
                    userId: "HongNguyen123",
    ...
    const faceSignIn = async () => {
        try {
            ...
            const loginResult = await UserService.loginUser({
                userName: formatName(userData.payload.username),
                pass: userData.facialId,
            });
