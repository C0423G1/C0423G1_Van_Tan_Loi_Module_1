...
public class BookingRepository implements IBookingRepository {
    ...
    public static final String DISPLAYEMAIL = "SELECT\n" +
            "    t.tour_name,\n" +
            "    t.start_date,\n" +
            "    t.end_date,\n" +
            "    DATEDIFF(t.end_date, t.start_date) AS date_range,\n" +
            "    b.quantity,\n" +
            "    e.employee_name,\n" +
            "    e.employee_phone,\n" +
            "    b.price * b.quantity AS total_amount\n" +
            "FROM\n" +
            "    tours t\n" +
    ...
    private static final String CHECKQUANTITY = "SELECT t.tour_id, t.tour_name, t.tour_available_seats, COALESCE(SUM(b.quantity), 0) AS total_booked_quantity FROM tours t LEFT JOIN bookings b ON t.tour_id = b.tour_id WHERE b.tour_id = ? AND b.customer_id = ? AND b.status = 1 GROUP BY t.tour_id HAVING t.tour_available_seats - COALESCE(SUM(b.quantity), 0) >= ?;\n";
    private static final String DISPLAYFROM = "SELECT t.tour_name, t.start_date, t.end_date, b.quantity, b.price, b.quantity * b.price AS total_price, t.tour_image FROM tours AS t INNER JOIN bookings AS b ON t.tour_id = b.tour_id WHERE b.status = 0 AND b.customer_id = ? AND b.quantity > 0;\n";
    ...
    public FromDto displayFrom(int customerId) throws SQLException {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                String img = resultSet.getString("tour_image");
    ...
    public List<TourEmailDto> displayEmail(int customerId) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            DecimalFormat decimalFormat = new DecimalFormat("#,##0");
            String formattedNumber = decimalFormat.format(tourSumPrice);
            arraylist.add(new TourEmailDto(tourName, startDate, endDate, time, quantity, customerName, customerPhone, formattedNumber));
        }
        return arraylist;
    }
